...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, NgbTypeaheadModule, JsonPipe],
...
export class NgbdtypeheadBasicComponent {
  ...
  search3: OperatorFunction<string, readonly { name: any; flag: any }[]> = (
    text$: Observable<string>
  ) =>
    text$.pipe(
      debounceTime(200),
  ...
  formatter3 = (x: { name: string }) => x.name;
  ...
  search4: OperatorFunction<string, readonly string[]> = (
    text$: Observable<string>
  ) => {
  ...
  search5: OperatorFunction<string, readonly string[]> = (
    text$: Observable<string>
  ) =>
    text$.pipe(
      debounceTime(300),
