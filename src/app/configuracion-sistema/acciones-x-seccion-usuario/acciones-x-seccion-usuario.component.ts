...
import { Datos } from './acciones-x-seccion-usuario';
import { AccionesxSeccionUsuarioService } from './acciones-x-seccion-usuario.service'; 
...
const appDatoSelector='app-acciones-x-seccion-usuario';
...
@Component({
  ...
  templateUrl: './acciones-x-seccion-usuario.component.html',
  styleUrls: ['./acciones-x-seccion-usuario.component.css'],
})
export class AccionesxSeccionUsuarioComponent implements OnInit {
  ...
  acciones = JSON.parse(localStorage.getItem('accion'));
  nombreSeccion = 'Permisos x usuario y seccion';
  nombreSeccionTabla = 'permisos_x_usuario_seccion';
  ...
  constructor(
    ///////////////////////////////////////////CAMBIAR ACA////////////////////////
    private Datoservice: AccionesxSeccionUsuarioService,
 ...
 /*  getDato(): void {
  ...
  filter(v: string) {
    return this.DatoList.filter(
      (x) => x.id_seccion.toLowerCase().indexOf(v.toLowerCase()) !== -1
    );
  }
...
/////////////////CAMBIAR ACA/////////////////////////////
      if (this.editDato != null) {
        this.DatoDetail.id_accion = this.editDato.get('id_accion')?.value;
        this.DatoDetail.id_seccion = this.editDato.get('id_seccion')?.value;
        this.DatoDetail.id_usuario = this.editDato.get('id_usuario')?.value;
